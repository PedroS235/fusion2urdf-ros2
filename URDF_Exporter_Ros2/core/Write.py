...
def write_link_urdf(
    joints_dict, repo, links_xyz_dict, file_name, inertial_dict
):
    """
        ...
        for j in joints_dict:
            ...
            try:
                xyz = [
                    round(p - c, 6)
                    for p, c in zip(
                        links_xyz_dict[parent], links_xyz_dict[child]
                    )
                ]  # xyz = parent - child
...
def write_urdf(
    ...
    file_name = (
        save_dir + "/urdf/" + robot_name + ".xacro"
    )  # the name of urdf file
...
def write_transmissions_xacro(
    ...
    file_name = save_dir + "/urdf/{}.trans".format(
        robot_name
    )  # the name of urdf file
    with open(file_name, mode="w") as f:
        f.write('<?xml version="1.0" ?>\n')
...
def write_gazebo_xacro(
    ...
    file_name = (
        save_dir + "/urdf/" + robot_name + ".gazebo"
    )  # the name of urdf file
...
def write_display_launch(package_name, robot_name, save_dir):
    ...
    file_text = launch_templates.get_display_launch_text(
        package_name, robot_name
    )
...
def write_gazebo_launch(package_name, robot_name, save_dir):
    ...
    file_text = launch_templates.get_gazebo_launch_text(
        package_name, robot_name
    )
